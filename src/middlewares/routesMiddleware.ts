...
import { isMatchingMarketParamRoute } from "./utils/marketCookieHelper";
...
export function withRoutesMiddleware(middleware: CustomMiddleware) {
  return async (request: NextRequest, event: NextFetchEvent, response: NextResponse) => {
    ...
    const currentMarketCookie = response.cookies.get(CookieNames.UserMarket)?.value;
    ...
    if ((isMarketParamPage || marketParam) && currentMarketCookie) {
      ...
      const matchedMarketParamRoute = Object.values(SwitchRoutesWithMarketParam).find(
        ...
        if (!marketParam || marketParam !== marketCookie.code) {
          if (searchParams.size > 0) {
            ...
            if (marketParam !== marketCookie.code) {
              ...
              return NextResponse.redirect(new URL(`${url.pathname}?${paramsUpdated}`, url));
            }
    ...
    if (matchingParamRoute) {
      ...
      if (searchParams.size === 0) {
        return NextResponse.redirect(
          new URL(`${url.pathname}?${matchingParamRoute.param}=${currentParamValue}`, url)
        );
      }
    }
    ...
    const matchingPrivatedRoute = privatedRoutesLists.flat().find((route) => {
      if (url.pathname.includes(route.path) && !request.cookies.has(route.cookieName)) {
        return route;
      }
    });
    ...
    const matchingAuthAvoidPublicRoute = AuthAvoidPublicRoutes.flat().find(
      (route) => route.path === url.pathname && request.cookies.has(route.cookieName)
    );
